	...
	while true do
		...
		if not IsPedInAnyVehicle(ped) then
			...
			if distance <= 2.5 then
				timeDistance = 4
				DrawText3D(dX,dY,dZ,"~g~E~w~   DELETAR PERSONAGEM")
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsEntityPlayingAnim(ped, "reaction@intimidation@1h","intro", 3) then
	...
	while true do
		  ...
		  if DoesEntityExist( ped ) and not IsEntityDead( ped ) and not IsPedInAnyVehicle(PlayerPedId(), true) then
				  ...
				  if CheckWeapon(ped) then
						  if holstered then
								  TaskPlayAnim(ped, "reaction@intimidation@1h", "intro", 5.0, 1.0, -1, 48, 0, 0, 0, 0 )
								  Citizen.Wait(1700)
								  ClearPedTasks(ped)
								  holstered = false
						  end
...
-----------------------------------------------------------------------------------------------------------------------------------------
local disService = {
	{ 311.6,-594.11,43.29,"Paramedic" }, -- Los Santos Cima
	{ 442.77,-981.41,30.69,"Police" }, -- Los Santos
	{ -344.0,-139.94,39.01,"Mechanic" }
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- DISTANCESERVICE
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("gcphone:submitCaller",function(data)
	...
	if message ~= nil and message ~= "" then
		TriggerServerEvent("gcphone:serverCaller",tostring(data.number),tostring(message))
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- FPS 
