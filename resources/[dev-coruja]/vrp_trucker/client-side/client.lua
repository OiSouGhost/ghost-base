				...
				if distance <= 3 then
					...
					if initStage then
						...
						if IsControlJustPressed(1,38) and distance <= 1 and not vSERVER.checkExist() and inSeconds <= 0 then
							...
							TriggerEvent("Notify","importante","Dirija-se até seu caminhão <b>Packer</b> e buzine o mesmo<br>e receba a carga responsável pelo transporte.",5000)
				...
				if distance <= 50 then
					...
					if IsControlJustPressed(1,38) and inSeconds <= 0 and distance <= 5 then
						...
						if modelVehicle == deliveryCoords[deliveryLocates][4] then
							TriggerEvent("Notify","importante","Volte ao ponto de partida e receba o pagamento.",5000)
							TriggerServerEvent("vrp_garages:DeleteVehicle",vehNet,vehPlate)
					...
					if GetEntityModel(veh) == 569305213 then
						...
						if IsControlJustPressed(1,38) and inSeconds <= 0 then
							...
							if HasModelLoaded(mHash) then
								...
								SetEntityAsMissionEntity(packageVehicle,true,false)
