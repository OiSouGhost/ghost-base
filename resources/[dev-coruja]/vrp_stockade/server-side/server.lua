			...
			if vRP.getInventoryItemAmount(user_id,stockadeItem) >= 1 then
				...
				if taskResult then
					if vRP.tryGetInventoryItem(user_id,stockadeItem,1,true) then
						...
						TriggerClientEvent("Notify",source,"sucesso","Sistema violado com sucesso e as autoridades foram notificadas.",5000)
				...
				TriggerClientEvent("Notify",source,"aviso","Você não pode roubar porque não possúi <b>"..vRP.itemNameList(stockadeItem).."</b>.",5000)
			end
		else
