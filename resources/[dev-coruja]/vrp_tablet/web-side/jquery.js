...
const midiaPage = (data) => {
	...
	$("#socialRight").html("")
	$("#socialRight").html("</div><div id='socialContent'></div>") 
	$.post("http://vrp_tablet/requestMedia",JSON.stringify({ media: data }),(data) => {
		$.each(JSON.parse(data),(k,v) => {
			$('#socialContent').prepend(`<div id="socialLine" ${v.back === true ? "style='background: ##fff; color: #fff;'\n":""}>${v.text}</div>`);
		})
	});
}
/* ---------------------------------------------------------------------------------------------------------------- */
const socialPage = () => {
	...
	$('#content').html(`
		<div id='titleContent'>TWITTAR...</div>
		<div id='socialLeft'>
			<textarea maxlength='280' class='mediaArea' spellcheck='false' value='' placeholder='Escreva um novo tweet...'></textarea>
			<div id='socialSubmit'>ENVIAR TWEET</div>
		</div>
...
const updateMedia = (media,data,back) => {
	if (mediaPage == media){
		$('#socialContent').prepend(`<div id="socialLine" ${back === true ? "style='background: ##fff; color: ##fff;'":""}>${data}</div>`);
	}
}
/* ---------------------------------------------------------------------------------------------------------------- */
const racesPage = (raceId) => {
	...
	$.post("http://vrp_tablet/requestRanking",JSON.stringify({ id: raceId }),(data) => {
		let position = 0;
...
const aboutPage = () => {
	...
	$.post("http://vrp_tablet/requestAbout",JSON.stringify({}),(data) => {
		...
		$("#content").html(`
			...
			<div id="aboutInfos">
				...
				<b>Coins:</b> <span id="aboutResult">${format(data.infos[3])}</span><br>
			</div>
		</div>
		`);
	});
...
$(document).on("click","#gameSubmit",function(e){
	...
	if (value > 0 && animal > 0){
		$.post("http://vrp_tablet/inputGame",JSON.stringify({ value: value, animal: animal }));
	}
});
/* ----------FORMAT---------- */
const format = (n) => {
