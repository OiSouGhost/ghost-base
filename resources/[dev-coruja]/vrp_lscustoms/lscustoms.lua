...
local garages = { 
	[1] = { locked = false, camera = { x = -582.85, y = -931.8, z = 23.89, heading = 89.74 }, driveout = { x = -582.85, y = -931.8, z = 23.89, heading = 89.74 }, drivein = { x = -582.85, y = -931.8, z = 23.89, heading = 89.74 }, outside = { x = -582.85, y = -931.8, z = 23.89, heading = 89.74 }, inside = { x = -582.85, y = -931.8, z = 23.89, heading = 89.74 }},
...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		AddMod(0,LSCMenu.categories,"AEROFOLIO","Aerofolio",nil,true)
		AddMod(3,LSCMenu.categories,"SAIAS-LATERAIS","Saias-laterais",nil,true)
		AddMod(4,LSCMenu.categories,"ESCAPAMENTO","Escapamento",nil,true)
		AddMod(6,LSCMenu.categories,"GRADE","Grade",nil,true)
		AddMod(7,LSCMenu.categories,"CAPO","Capo",nil,true)
		AddMod(8,LSCMenu.categories,"PARA-LAMA","Para-lama",nil,true)
		AddMod(10,LSCMenu.categories,"TETO","Teto",nil,true)
		AddMod(12,LSCMenu.categories,"FREIO","Freio",nil,true)
		AddMod(13,LSCMenu.categories,"TRANSMISSÃO","Transmissão",nil,true)
		AddMod(14,LSCMenu.categories,"BUZINA", "Buzina",nil,true)
		AddMod(15,LSCMenu.categories,"SUSPENSÃO","Suspensão",nil,true)
		AddMod(16,LSCMenu.categories,"BLINDAGEM","Blindagem",nil,true)
		AddMod(18, LSCMenu.categories,"TURBO","Turbo",nil,true)
		
		if chassis then
			LSCMenu.categories:addSubMenu("CHASSIS","Chassis",nil,true)
			AddMod(42, LSCMenu.categories.Chassis,"COBERTURA DE RODAS","Cobertura de Rodas",nil,true)
			AddMod(43, LSCMenu.categories.Chassis,"ANTENAS","Antena",nil,true)
			AddMod(44, LSCMenu.categories.Chassis,"VENTILAÇÃO","Ventilação",nil,true)
			AddMod(45, LSCMenu.categories.Chassis,"TANQUE","Tanque",nil,true)
			AddMod(46, LSCMenu.categories.Chassis,"PORTAS","Portas",nil,true)
			AddMod(5,LSCMenu.categories.Chassis,"GAIOLA DE PROTEÇÃO","Gaiola de Proteção",nil,true)
		end
									...
									if pos.locked then
										if not LSC_Config.lock then
											if IsControlJustPressed(1,38) then
												...
												drawTxt("PRESSIONE  ~r~E~w~  PARA ACESSAR A ~y~BENNYS",4,0.5,0.93,0.50,255,255,255,180)
											end
										else
										...
										if IsControlJustPressed(1,38) then
											...
											drawTxt("PRESSIONE  ~r~E~w~  PARA ACESSAR A ~y~MECANICA",4,0.5,0.93,0.50,255,255,255,180)
										end
									end
								else
									drawTxt("~y~BENNYS~w~ OCUPADA NO MOMENTO",4,0.5,0.93,0.50,255,255,255,180)
...
AddEventHandler("LSC:buttonSelected",function(name,button,canpurchase)
	...
	elseif mname == "liveries" or mname == "hydraulics" or mname == "buzina" or mname == "tanque" or mname == "ornaments" or  mname == "Cobertura de rodas" or mname == "antena" or mname == "ventilação" or mname == "portas" or mname == "gaiola de proteção" or mname == "engine block" or mname == "cam cover" or mname == "strut brace" or mname == "trim design" or mname == "ormnametns" or mname == "dashboard" or mname == "dials" or mname == "seats" or mname == "steering wheels" or mname == "plate holder" or mname == "vanity plates" or mname == "shifter leavers" or mname == "plaques" or mname == "speakers" or mname == "trunk" or mname == "blindagem" or mname == "suspensão" or mname == "transmissão" or mname == "freio" or mname == "engine tunes" or mname == "teto" or mname == "capo" or mname == "grade" or mname == "gaiola de proteção" or mname == "escapamento" or mname == "saias-laterais" or mname == "rear bumpers" or mname == "front bumpers" or mname == "aerofolio" then
		if button.name == "Stock" or button.purchased or CanPurchase(price,canpurchase)then
			myveh.mods[button.modtype].mod = button.mod
			SetVehicleMod(veh,button.modtype,button.mod)
		end
	elseif mname == "para-lama" then
		if button.name == "Stock" or button.purchased or CanPurchase(price,canpurchase)then
...
function LSCMenu:OnMenuChange(last,current)
	...
	elseif c == "portas" and last.name:lower() == "interior" then
...
function CheckPurchases(m)
	...
	elseif name == "blindagem" or name == "suspensão" or name == "transmissão" or name == "freio" or name == "engine tunes" or name == "teto" or name == "para-lama" or name == "capo" or name == "grade" or name == "gaiola de proteção" or name == "escapamento" or name == "saias-laterais" or name == "rear bumpers" or name == "front bumpers" or name == "aerofolio" then
		for i,b in pairs(m.buttons) do
			if b.mod == -1  then
	...
	elseif name == "neon layout" then
		for i,b in pairs(m.buttons) do
			...
			elseif b.name == "Front,Back and Sides" then
				if IsVehicleNeonLightEnabled(myveh.vehicle,0) == true and IsVehicleNeonLightEnabled(myveh.vehicle,1) == true and IsVehicleNeonLightEnabled(myveh.vehicle,2) == true and IsVehicleNeonLightEnabled(myveh.vehicle,3) == true then
					b.sprite = "garage"
				else
					b.sprite =  nil
				end
	...
	elseif name == "tanque" or name == "ornaments" or name == "Cobertura de rodas" or name == "antena" or name == "ventilação" or name == "portas" or name == "gaiola de proteção" or name == "engine block" or name == "cam cover" or name == "strut brace" or name == "trim design" or name == "ornametns" or name == "dashboard" or name == "dials" or name == "seats" or name == "steering wheels" or name == "plate holder" or name == "vanity plates" or name == "shifter leavers" or name == "plaques" or name == "speakers" or name == "trunk" or name == "headlights" or name == "turbo" or  name == "hydraulics" or name == "liveries" or name == "buzina" then
		for i,b in pairs(m.buttons) do
			if myveh.mods[b.modtype].mod == b.mod then
				b.sprite = "garage"
			else
