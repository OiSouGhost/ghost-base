...
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local emX,emY,emZ = 40.58,-1000.84,29.41
		...
		while true do
			if inService and inTimers > 0 then
				...
				if inTimers <= 0 then
					...
					TriggerEvent("NotifyPush",{ code = "QTH",text = "Ola gostaria de estar comprando este produto.", title = "Quero comprar um produto", x = pedLocate[rand][1], y = pedLocate[rand][2], z = pedLocate[rand][3], name = callName[math.random(#callName)].." "..callName2[math.random(#callName2)], rgba = {75,100,160} })
				end
			end
