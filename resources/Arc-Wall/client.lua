...
arc = Tunnel.getInterface("Arc-Wall")
Config = {}
...
RegisterCommand("blips",function()
    ...
    if wall then
		drawNotification("~y~Blips ativado")
		arc.reportwallLog("WALL ON")
		print("^2WALL ON")
		PlaySoundFrontend(-1, "CONFIRM_BEEP", "HUD_MINI_GAME_SOUNDSET", 0)
	else
		drawNotification("~r~Blips desativados")
		arc.reportunwallLog("WALL OFF")
		print("^1WALL OFF")
	end
end)
					...
					if ((NetworkIsPlayerActive( id )) and GetPlayerPed(id) ~= PlayerPedId()) then
						...
						if admin and (PlayerPedId() ~= GetPlayerPed(id)) then
							if GetPlayerPed(id) ~= -1 and players[id] ~= nil then
								...
								if distance <= Config.Distanciaesp then
									HazeStore = 1
									DrawText3D(x2, y2, z2+1.7,"⭐~y~ "..name.." ~w~["..players[id].."]")
								end
							end
						end
					end
...
-- Config Das Linhas
----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("linhas",function()
	if not admin then
		return
	end
	...
	if lines then
		drawNotification("~y~Linhas ativadas")
		arc.reportlinesLog("LINHAS ON")
		print("^2LINHAS ON!")
		PlaySoundFrontend(-1, "CONFIRM_BEEP", "HUD_MINI_GAME_SOUNDSET", 0)
	else
		drawNotification("~r~Linhas desativadas")
		arc.reportunlinesLog("LINHAS OFF")
		print("^1LINHAS OFF!")
	end
end)
----------------------------------------------------------------------------------------------------------------------------------------
			...
			if lines then
				for k, id in ipairs(GetActivePlayers()) do
					if ((NetworkIsPlayerActive( id )) and GetPlayerPed(id) ~= PlayerPedId()) then	
						if admin and (PlayerPedId() ~= GetPlayerPed(id)) then
							if GetPlayerPed(id) ~= -1 and players[id] ~= nil then
								local getplayersonline = GetActivePlayers()
								x1, y1, z1 = table.unpack( GetEntityCoords( PlayerPedId(), true ) )
						        x2, y2, z2 = table.unpack( GetEntityCoords( GetPlayerPed( id ), true ) )
								local distance = math.floor(GetDistanceBetweenCoords(x1,  y1,  z1,  x2,  y2,  z2,  true))	
								local gete, entityc, cordes = table.unpack(GetEntityCoords(PlayerPedId()))
								local x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(id)))
								if distance <= Config.Distanciaesp then
									HazeStore = 1
									DrawLine(gete, entityc, cordes, x, y, z, 255, 255, 255, 255)
								end
								if distance <= Config.Distanciaesp and not IsEntityVisible(GetPlayerPed(id)) then
									DrawLine(gete, entityc, cordes, x, y, z, 255, 0, 0, 255)
								end
							end
						end
					end
